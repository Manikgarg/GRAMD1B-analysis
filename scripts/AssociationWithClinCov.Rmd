...
pfs$Event<-"PFS"
os$Group<- factor(c("GRAMD1B", 
             "GRAMD1B+\nStage (II vs III)"), levels=c("GRAMD1B", 
             "GRAMD1B+\nStage (II vs III)"))
pfs$Group<- factor(c("GRAMD1B", 
             "GRAMD1B+\nStage (II vs III)"), levels=c("GRAMD1B", 
             "GRAMD1B+\nStage (II vs III)"))
...
g3<-ggplot(data=df,
    ...
    ylab("Hazard Ratio (95% Confidence Interval)")+
    geom_errorbar(aes(ymin=as.numeric(Lower_HazardRatio), ymax=as.numeric(Upper_HazardRatio)),width=0.1,cex=1, position=position_dodge(width=0.5))+ 
    geom_text(hjust = 0.75, vjust=-0.75, position=position_dodge(width=0.5), size=2.5,  family="sans")+
    #scale_y_continuous()+
    ylim(c(0,2))+
    scale_color_brewer(type = "qual", palette = "Set2")+
    coord_flip()+
...
ggsave(paste("../results/survival/AVAST-M_",tissue,"_OS_PFS_ForestPlots_Stage_binary.pdf", sep=""), device = "pdf", width = 8, units = "cm", height=6)
