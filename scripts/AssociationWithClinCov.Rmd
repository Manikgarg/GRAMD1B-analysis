...
#write.table(expressionData, "~/Desktop/Melanoma/AVAST-M_Skin-Primaries_VST_normalized_expression_data.csv", sep = ",", quote = F, row.names = T, col.names = T)
```

```{r}
#expressionData <- data.frame(assay(vsd))
#write.table(data.f, "~/Desktop/Melanoma/AVAST-M_Regional-Lymph-Node_raw_expression_data.csv", sep = ",", quote = F, row.names = T, col.names = T)
```
...
ggsave(paste("../results/AVAST-M_primaries_Stage_substages.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```
...
ggplot(clinicalData[!is.na(clinicalData$Stage_binary),], aes(x=Stage_binary, y=Signature))+
  ...
  #ggpubr::stat_compare_means(label.y = 9, method = "anova", size = 2.5, family="sans")
ggsave(paste("../results/AVAST-M_primaries_Stage_binary.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```
...
ggsave(paste("../results/AVAST-M_primaries_BRAF.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
...
ggplot(clinicalData[!is.na(clinicalData$NRAS),], aes(x=NRAS, y=Signature))+
  ...
  theme(text=element_text(size=7,  family="sans"))+
  ggpubr::stat_compare_means(comparisons = my_comparisons, method = "t.test", size = 2.5, family="sans")
ggsave(paste("../results/AVAST-M_primaries_NRAS.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
#+ # Add pairwise comparisons p-value
  #ggpubr::stat_compare_means(label.y = 9, method = "anova", size = 2.5, family="sans")#+
```
#With PTEN
...
ggplot(clinicalData[!is.na(clinicalData$PTENGroupMean),], aes(x=PTENGroupMean, y=Signature))+
  ...
  theme(text=element_text(size=7,  family="sans"))+
  ggpubr::stat_compare_means(comparisons = my_comparisons, method = "t.test", size = 2.5, family="sans")
ggsave(paste("../results/AVAST-M_primaries_PTEN.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```

#With BRAS mutant PTEN low
```{r}
clinicalData$BRAF_PTEN<-ifelse(
   (clinicalData$BRAF=="V600E") & (clinicalData$PTENGroupMean=="Low"),
   "Yes",
   "No")
...
ggplot(clinicalData[!is.na(clinicalData$BRAF_PTEN),], aes(x=BRAF_PTEN, y=Signature))+
  ...
  theme(text=element_text(size=7,  family="sans"))+
  ggpubr::stat_compare_means(comparisons = my_comparisons, method = "t.test", size = 2.5, family="sans")
ggsave(paste("../results/AVAST-M_primaries_BRAF_PTEN.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```
...
HALLMARK_TNFA_SIGNALING_VIA_NFKB_genes<-c('CCL2','IL1A','BIRC3','PTX3','CXCL1','LIF','IRS2','RCAN1','CSF1','INHBA','BMP2','MAFF','TNFSF9','GADD45A','SERPINB8','IL1B','ICAM1','DUSP2','HBEGF','BCL2A1','CD44','BIRC2','IL23A','PLPP3','RELB','CCL20','TNFAIP3','EGR2','PPP1R15A','TRAF1','TNFRSF9','EDN1','CD83','SQSTM1','EGR3','IFIH1','YRDC','MAP3K8','PLAUR','RNF19B','SAT1','KYNU','CXCL3','TAP1','LAMB3','FOSL1','NFKBIE','KLF2','TUBB2A','SDC4','MAP2K3','FJX1','NR4A3','CXCL10','F3','EGR1','CSF2','TRIB1','BTG3','IFNGR2','GCH1','EIF1')
...
```
## Plot univariate survival curves
```{r}
library(survminer)
#Overall survival
fit = survfit(survival_d_ltrc ~ as.factor(SignatureGroupMean), data = clinicalData)
...
survivalModel<-coxph(survival_d_ltrc~as.factor(SignatureGroupMean), data = clinicalData)
...
pdf(paste("~/Desktop/Melanoma/Shruthy/GRAMD1B/results/survival/AVAST-M_",tissue,"_GRAMD1B_OS_K-M_curve.pdf", sep=""), width = 3.15, height = 3.15, family="sans", pointsize=7)
...
```{r}
fit = survfit(survival_rd_ltrc ~ as.factor(SignatureGroupMean), data = clinicalData)
# g1<- autoplot(fit, pval = TRUE)+
#    ylab("Survival")+
#    scale_color_manual(values = c("#E31A1C", "#1F78B4"), name = "Group")+
#    scale_fill_manual(values = c("#E31A1C", "#1F78B4"), name = "Group")

survivalModel<-coxph(survival_rd_ltrc~as.factor(SignatureGroupMean), data = clinicalData)
x <- summary(survivalModel)
pvalStr<- signif(x$coef[1, ncol(x$coef)], digits = 5)
#survminer::surv_pvalue(fit = fit, data = clinicalData)$pval
# KM curve for signature
pdf(paste("~/Desktop/Melanoma/Shruthy/GRAMD1B/results/survival/AVAST-M_",tissue,"_GRAMD1B_PFS_K-M_curve.pdf", sep=""), width = 3.15, height = 3.15, family="sans", pointsize=7)
plot(fit, col = c("blue", "red"), xlab = "Time (years)", ylab = "Survival probability", lwd=2)
if(pvalStr>=0.0001){
   pvalStr=round(pvalStr, digits = 4)
} else{
...
text(x= 2, y=0.05, paste0("p=", pvalStr))
legend("topright", col = c("blue", "red"), legend = c("High", "Low"), lty = 1, lwd=2)
dev.off()
```

## then calculate the values for multivariate survival analysis and make forest plot
```{r}
os<-data.frame()
pfs<-data.frame()
temp<-data.frame()
...
if (tissue=="Skin") {
  if(!("ClarkScore"%in%colnames(clinicalData))){
    jt_ns_sm_scores = read.table("~/Desktop/Melanoma/JT_NS_SM_Combined.xls - JT_NS_SM.tsv", sep = "\t", header = TRUE, quote = "")
  combinedScore <- data.frame("Scanned_file.me"= jt_ns_sm_scores$JT_Scanned_file.me,
                              "R.Seq_sampleID"= jt_ns_sm_scores$JT_R.Seq_sampleID,
                              "ClarkScore"=ifelse(jt_ns_sm_scores$JT_Clark_score==jt_ns_sm_scores$NS_Clark_score,
                                                  jt_ns_sm_scores$JT_Clark_score,
  ...
  combinations<-c("", 
                ...
                "+as.numeric(Nclass)",
                "+NRAS",
                "+Sex+Age+as.character(Stage)+ECOG+treatment+as.numeric(Nclass)+NRAS",
                "+Sex+Age+as.character(Stage)+ECOG+treatment+as.numeric(Nclass)+NRAS+ClarkScore")}else{
                  combinations<-c("", 
                "+Sex",
                "+Age",
                "+as.character(Stage)",
...
for (combination in combinations) {
  #Add TIL score if tissue = Skin, otherwise, omit
    os_formula<-paste0("survival_d_ltrc~Signature",combination, collapse = "")
    pfs_formula<-paste0("survival_rd_ltrc~Signature",combination, collapse = "")
  #Overall survival
  ...
  high = exp(fit[1,c("coef")]+
              qnorm(.975)*fit[1,c("se(coef)")])  
  pval = fit[1,c("Pr(>|z|)")]
  temp["Signature",c("Group", "Hazard_Ratio","Lower_HazardRatio","Upper_HazardRatio","p_value")] = c(paste0("GRAMD1B",combination, collapse = ""),mid,low,high,pval)
  os<-rbind(os, temp)
...
#Forest plot for OS and PFS
os$Event<-"OS"
pfs$Event<-"PFS"
if (tissue=="Skin") {
  os$Group<- factor(c("GRAMD1B", 
             "GRAMD1B+Sex", 
             "GRAMD1B+Age", 
             "GRAMD1B+Stage", 
             "GRAMD1B+ECOG", 
             ...
             "GRAMD1B+Nclass",
             "GRAMD1B+NRAS",
             "GRAMD1B+Sex+Age+Stage+\nECOG+treatment+Nclass+NRAS",
             "GRAMD1B+Sex+Age+Stage+\nECOG+treatment+Nclass+NRAS+\nTIL count"), levels=c("GRAMD1B", 
             "GRAMD1B+Sex", 
             "GRAMD1B+Age", 
             "GRAMD1B+Stage", 
             "GRAMD1B+ECOG", 
  ...
  os$Group<- factor(c("GRAMD1B", 
             ...
             "GRAMD1B+Nclass",
             "GRAMD1B+NRAS",
             "GRAMD1B+Sex+Age+Stage+\nECOG+treatment+Nclass+NRAS"), levels=c("GRAMD1B", 
             "GRAMD1B+Sex", 
             "GRAMD1B+Age", 
             "GRAMD1B+Stage", 
             "GRAMD1B+ECOG", 
...
g3<-ggplot(data=df,
    ...
    geom_text(hjust = 0.5, vjust=-2, position=position_dodge(width=0.5), size=2.5,  family="sans")+
    #scale_y_continuous()+
    ylim(c(0,4))+
...
ggsave(paste("../results/survival/AVAST-M_",tissue,"_OS_PFS_ForestPlots_withNRAS.pdf", sep=""), device = "pdf", width = 10, units = "cm", height=12)


#Save the values
#write.table(os, paste("../results/survival/AVAST-M_",tissue,"_OS_multivariate_withNRAS.tsv", sep=""), sep="\t", col.names = T, row.names = F)
#write.table(pfs, paste("../results/survival/AVAST-M_",tissue,"_PFS_multivariate_withNRAS.tsv", sep=""), sep="\t", col.names = T, row.names = F)
```
...
if (tissue=="Skin") {
  ...
  combinations<-c("", 
                "+Sex",
                "+Age",
                "+as.character(Stage_binary)",
                "+ECOG",
                "+treatment",
                "+as.numeric(Nclass)",
                "+NRAS",
                "+Sex+Age+as.character(Stage_binary)+ECOG+treatment+as.numeric(Nclass)+NRAS",
                "+Sex+Age+as.character(Stage_binary)+ECOG+treatment+as.numeric(Nclass)+NRAS+ClarkScore")}else{
                  combinations<-c("", 
                "+Sex",
                "+Age",
                "+as.character(Stage_binary)",
...
if (tissue=="Skin") {
  os$Group<- factor(c("GRAMD1B", 
             "GRAMD1B+Sex", 
             "GRAMD1B+Age", 
             "GRAMD1B+Stage (II vs III)", 
             "GRAMD1B+ECOG", 
             "GRAMD1B+Treatment", 
             "GRAMD1B+Nclass",
             "GRAMD1B+NRAS",
             "GRAMD1B+Sex+Age+\nStage (II vs III)+ECOG+\ntreatment+Nclass+NRAS",
             "GRAMD1B+Sex+Age+\nStage (II vs III)+ECOG+\ntreatment+Nclass+NRAS+\nTIL count"), levels=c("GRAMD1B", 
             "GRAMD1B+Sex", 
             "GRAMD1B+Age", 
             "GRAMD1B+Stage (II vs III)", 
  ...
  os$Group<- factor(c("GRAMD1B", 
             ...
             "GRAMD1B+Nclass",
             "GRAMD1B+NRAS",
             "GRAMD1B+Sex+Age+\nStage (II vs III)+ECOG+\ntreatment+Nclass+NRAS"), levels=c("GRAMD1B", 
             "GRAMD1B+Sex", 
             "GRAMD1B+Age", 
             "GRAMD1B+Stage (II vs III)", 
             "GRAMD1B+ECOG", 
...
ggsave(paste("../results/survival/AVAST-M_",tissue,"_OS_PFS_ForestPlots_withNRAS_Stage_binary.pdf", sep=""), device = "pdf", width = 10, units = "cm", height=14)
