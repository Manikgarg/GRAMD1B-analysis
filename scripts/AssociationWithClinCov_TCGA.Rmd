...
### Merge mutation information with clinical data
```{r}
...
ggsave(paste("../results/TCGA_regional_primaries_BRAF.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```
...
ggplot(clinicalData[!is.na(clinicalData$NRAS),], aes(x=NRAS, y=Signature))+
  ...
  theme(text=element_text(size=7,  family="sans"))+
  ggpubr::stat_compare_means(comparisons = my_comparisons, method = "t.test", size = 2.5, family="sans")
ggsave(paste("../results/TCGA_regional_primaries_NRAS.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```
...
ggplot(clinicalData[!is.na(clinicalData$PTENGroupMean),], aes(x=PTENGroupMean, y=Signature))+
  ...
  theme(text=element_text(size=7,  family="sans"))+
  ggpubr::stat_compare_means(comparisons = my_comparisons, method = "t.test", size = 2.5, family="sans")
ggsave(paste("../results/TCGA_regional_primaries_PTEN.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```
...
ggplot(clinicalData[!is.na(clinicalData$BRAF_PTEN),], aes(x=BRAF_PTEN, y=Signature))+
  ...
  theme(text=element_text(size=7,  family="sans"))+
  ggpubr::stat_compare_means(comparisons = my_comparisons, method = "t.test", size = 2.5, family="sans")
ggsave(paste("../results/TCGA_regional_primaries_BRAF_PTEN.pdf", sep=""), device = "pdf", width =8, units = "cm", height=6)
```
```{r}
...
pdf(paste("~/Desktop/Melanoma/Shruthy/GRAMD1B/results/survival/TCGA_", tissue, "_OS_K-M_curve.pdf", sep=""), width = 3.15, height = 3.15, family="sans", pointsize=7)
...
g1 <- ggplot(data=tcga_surv,
   ...
   theme(text=element_text(size=7,  family="sans"))
   #theme_set(ggpubr::theme_pubr(base_size=10, legend='bottom'))
ggsave(paste("~/Desktop/Melanoma/Shruthy/GRAMD1B/results/survival/TCGA_", tissue, "_OS_ForestPlots.pdf", sep=""), device = "pdf", width = 10, units = "cm", height=4.5)
```
...
fit <- survfit( survival_d_rc ~ SignatureGroupMean_breslow, 
      data = clinicalData )
# KM curve for signature
pdf(paste("~/Desktop/Melanoma/Shruthy/GRAMD1B/results/survival/TCGA_", tissue, "_OS_K-M_curve_breslow.pdf", sep=""), width = 3.15, height = 3.15, family="sans", pointsize=7)
plot(fit, col = c("blue", "red"), xlab = "Time (years)", ylab = "Survival probability", lwd=2)
pvalStr = survminer::surv_pvalue(fit = fit, data = clinicalData)$pval
if(pvalStr>=0.0001){
   pvalStr=round(pvalStr, digits = 4)
